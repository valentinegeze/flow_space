...
function buildChannelMask(data, w, h, params) {
  ...
  // Per-class pixel counters (for particle-size calibration of the patch model)
  let darkCount = 0;        // black 0.7 mm
  let redCount = 0;         // red 0.4 mm
  let wetCoarseCount = 0;   // yellow 1.4 mm + white 1.1 mm
  ...
  const tableArea = (w - 2 * mx) * (h - 2 * my);
  return {
    mask,
    classCounts: {
      darkFraction:      tableArea > 0 ? darkCount / tableArea : 0,
      redFraction:       tableArea > 0 ? redCount / tableArea : 0,
      wetCoarseFraction: tableArea > 0 ? wetCoarseCount / tableArea : 0,
    },
  };
}
...
function analyzeFrame(data, w, h, params) {
  ...
  const { mask: rawMask, classCounts } = buildChannelMask(data, w, h, params);
  ...
  return {
    channelFraction, largestComponentFraction, nComponents, spanning, compMap, binaryMask,
    darkFraction:      classCounts.darkFraction,
    redFraction:       classCounts.redFraction,
    wetCoarseFraction: classCounts.wetCoarseFraction,
  };
}
...
self.onmessage = function(e) {
  ...
  if (msg.type === 'analyze') {
    ...
    self.postMessage({
      ...
      darkFraction: result.darkFraction,
      redFraction: result.redFraction,
      wetCoarseFraction: result.wetCoarseFraction,
